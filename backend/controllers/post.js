...
exports.getAllPost = (req, res, next) => { //Retourne un tableau des posts dans l'ordre décroissant des dates de publication
    Post.findAll({
            ...
            include: [{
                model : User,
                attributes: ['id_user', 'firstname', 'lastname' , 'avatar']
            }],
...
exports.createPost = (req, res, next) => {
    Post.create({
            ...
            image_url: req?.file!=null?`${req.protocol}://${req.get('host')}/images/posts/${req?.file?.filename}`:undefined,
    ...
    Post.findOne({
        ...
        .then((post) => {
            if(post.image_url){
                ...
                fs.unlink(`images/posts/${filename}`, () => {console.log('Fichier image supprimée')});
            }
