<template>
    <div v-if="session" class="div-global">
        ...
        <section>
            <div>
                ...
                <form v-on:submit="modifyProfile">
                    ...
                    >
                    <h3>Changez votre avatar si vous le souhaitez</h3>
        ...
        <modaldeleteaccount v-if="modal" v-on:close="modal=false"></modaldeleteaccount>
    </div>
</template>
...
import ModalDeleteAccount from './layouts/ModalDeleteAccount.vue';
...
export default {
    ...
    components: {
     ...
     computed: {
        btnDisabled(){
            if(this.valid.firstname && this.valid.lastname != ''){
                return false;
            }else return true;
        }
    },
    methods:{
        ...
        validText(text, input){
            const regexText = new RegExp(/^[a-zA-Z\s'\-àáâãäæçèéêëìíîïñòóôõöùúûüýÿœÀÁÂÃÄÆÇÈÉÊËÌÍÎÏÑÒÓÔÕÖÙÚÛÜÜŒ]+$/);
